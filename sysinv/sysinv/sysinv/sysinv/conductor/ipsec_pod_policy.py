...
class IpsecPodPolicyOperator(object):
    ...
    def _get_node_pod_cidr(self):
        try:
            ...
            LOG.error("failed try to get blockaffinities from "
                      f"kubernetes: {e}")
            raise e
    ...
    def _get_node_cluster_ip(self):
        try:
            ...
            if len(self.k8s_host_dict) != len(nodes):
                err_msg = ("Error, node number not match with node pod "
            ...
            LOG.error(f"failed try to get cluster IP from kubernetes: {e}")
            raise e
    ...
    def _update_host_dict_from_kubernetes(self):
        # Add retry here since a new node's affinity data will delay
        # present from K8S.
        for attempt in range(1, RPC_RETRY_TIMES + 1):
            try:
                self.k8s_host_dict = dict()
                self._get_node_pod_cidr()
                self._get_node_cluster_ip()
            except Exception as e:
                LOG.error(f"Attempt {attempt} failed try to get host info"
                          f" from kubernetes: {e}")
                if attempt < RPC_RETRY_TIMES:
                    time.sleep(10)
                else:
                    raise
    ...
    def _rpc_call_agent(self, context, host_uuid, data,
                        retry_times=RPC_RETRY_TIMES):
    ...
    def _wait_host_ready_apply_pod_policy(self, context, host):
        ...
        while True:
            ...
            if self.policy_apply_lock.locked():
                LOG.info("wait for previous IPsec pod policy apply task "
                         "end...")
                continue
            ...
            if ret is None:
                ...
                times += 1
                if times * 10 >= WAIT_ONLINE_TIMEOUT:
