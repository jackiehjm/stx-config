...
class IpsecPodPolicyOperator(object):
    ...
    def _get_node_pod_cidr(self):
        max_retries = 3
        # add retry here, there is a bit change call kubernetes API failed
        for attempt in range(1, max_retries + 1):
            try:
                ...
                LOG.error(f"Attempt {attempt} failed try to get blockaffinities"
                          f" from kubernetes: {e}")
    ...
    def _get_node_cluster_ip(self):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                LOG.error(f"Attempt {attempt} failed try to get nodes address"
                          f" from kubernetes: {e}")
                if attempt < max_retries:
                    time.sleep(5)
                else:
