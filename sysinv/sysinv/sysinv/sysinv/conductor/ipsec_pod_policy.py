...
class IpsecPodPolicyOperator(object):
    ...
    def _validate_host(self, host):
        ...
        if not (host['administrative'] == constants.ADMIN_UNLOCKED and
                host['availability'] == constants.AVAILABILITY_AVAILABLE):
            LOG.info("skip host %s for not in unlock and avaliable "
    ...
    def _get_avaliable_hosts(self):
        ...
        for host in hosts:
            if self._validate_host(host):
                host_name_list.append([host.hostname, host.uuid])
            else:
    ...
    def _apply_policy_on_hosts(self, context, hosts):
        ...
        for host_name, host_uuid in hosts:
            ...
            ret = rpcapi.apply_ipsec_pod_policy_manifest(context, host_uuid,
                                                         data)
            if ret:
    ...
    def _remove_all_policy_on_hosts(self, context, hosts):
        rpcapi = agent_rpcapi.AgentAPI()
        err_list = list()
        for host_name, host_uuid in hosts:
            if host_name not in self.k8s_host_dict:
    ...
    def _wait_host_ready_apply_pod_policy(self, context, host):
        while self._apply_condition(host.uuid):
            time.sleep(10)
            LOG.info("wait for host %s unlock and avaliable to apply pod "
                     "policy" % host.hostname)
        LOG.info("host %s in unlock and avaliable, ready to apply policy"
                 % host.hostname)
    ...
    def apply_ipsec_pod_policy_config(self, context):
        ...
        with self.policy_apply_lock:
            ...
            hosts = self._get_avaliable_hosts()
            LOG.info(f"apply ipsec pod policy on hosts: {hosts}")
            if not hosts:
            ...
            if not system.capabilities.get('pod_to_pod_security_enabled',
                ...
                self._remove_all_policy_on_hosts(context, hosts)
                return err_msg
            ...
            err_list = self._apply_policy_on_hosts(context, hosts)
            if err_list:
                return str(err_list)
            return err_msg
